    ...
    return (
      <>
        ...
        <main className="flex flex-1 flex-col gap-4">
          <h1 className="app-title text-[var(--text)]">Gilabun</h1>
  ...
  return (
    <>
      ...
      <main className="flex flex-1 flex-col gap-8">
        <div>
          <h1 className="text-xl font-semibold text-[var(--text)]">Gilabun</h1>
          <p className="mt-1 text-sm text-[var(--muted)]">
            Start a new workspace for a service session, or open an existing one.
          </p>
        </div>
                ...
                >
                  <div>
                    ...
                    <span className="ml-2 meta-text text-[var(--muted)]">
                      {w.templateName} · {w.createdAt.toLocaleDateString()} {w.createdAt.toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit" })}
                    </span>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <form action={deleteWorkspaceFormAction} className="contents">
                      <input type="hidden" name="workspaceId" value={w.id} />
                      <button type="submit" className="btn-ghost text-sm text-[var(--muted)] hover:text-[var(--red)]">
                        Delete
                      </button>
                    </form>
                  </div>
        ...
        <section>
          ...
          <StartNewWorkspaceForm templates={templates} />
        </section>
