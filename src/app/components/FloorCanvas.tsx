      ...
      >
        ...
        {tablesRender.map((item) => {
          ...
          return (
            <div
              key={item.tableNumber}
              className={`absolute flex items-center justify-center rounded-none ${isSelected ? "ring-2 ring-[var(--primary-action)] ring-offset-1 ring-offset-[var(--panel)]" : ""}`}
              style={{
                left: item.renderX,
                top: item.renderY,
                width: item.renderW,
        ...
        {combos.map((combo) => {
          ...
          const isComboOverdue =
            ...
            Date.now() >= (combo.expectedFreeAt instanceof Date ? combo.expectedFreeAt.getTime() : new Date(combo.expectedFreeAt).getTime());
          const comboColor =
            combo.status === "FREE"
              ? "table-free"
